...
export default function WomenStoriesPage() {
    ...
    return (
        <section className="relative overflow-hidden bg-linear-to-b from-[#120f20] via-[#170f24] to-[#251224] py-24">
            {/* Background Effects */}
            ...
            <div className="relative z-10 mx-auto max-w-7xl px-6">
                ...
                <div className="mb-10 flex justify-center gap-4">
                    <button
                        ...
                        className={`px-8 py-3 rounded-lg font-semibold text-sm uppercase tracking-widest transition-all duration-300 ${activeVolume === "vol1" ? "bg-linear-to-r from-pink-500 to-violet-500 text-white shadow-lg shadow-pink-500/30" : "bg-white/5 border border-white/20 text-white/70 hover:text-white hover:border-white/40 hover:bg-white/8"}`}
                    >
                        Volume 1
                    </button>
                    <button
                        onClick={() => setActiveVolume("vol2")}
                        className={`px-8 py-3 rounded-lg font-semibold text-sm uppercase tracking-widest transition-all duration-300 ${activeVolume === "vol2" ? "bg-linear-to-r from-pink-500 to-violet-500 text-white shadow-lg shadow-pink-500/30" : "bg-white/5 border border-white/20 text-white/70 hover:text-white hover:border-white/40 hover:bg-white/8"}`}
                    >
                        Volume 2
                    </button>
                </div>
                ...
                {!isLoading && !error && stories.length > 0 && (
                    <div className="grid gap-8 lg:grid-cols-3">
                        {stories.map((item) => (
                            <div
                                key={item.id}
                                className="group relative overflow-hidden rounded-4xl border border-white/10 bg-white/3 backdrop-blur-xl transition-all duration-500 hover:-translate-y-2 hover:border-pink-400/30 hover:shadow-[0_20px_60px_rgba(236,72,153,0.15)]"
                            >
                                {/* WOMAN CARD */}
                                {item.type === "woman" ? (
                                    <>
                                        ...
                                        <div className="relative p-8">
                                            ...
                                            {/* Divider Line */}
                                            <div className="mt-5 h-px w-16 bg-linear-to-r from-pink-400 to-transparent" />

                                            {/* Story Text */}
                                            <p
                                                className="mt-5 text-white/65 leading-relaxed transition-all duration-300 group-hover:text-white/85"
                                            >
                                                {item.story}
                                            </p>
                                        </div>
                                    ...
                                    <>
                                        ...
                                        <div className="relative flex min-h-155 flex-col justify-center p-10">
                                            ...
                                            <Quote
                                                size={150}
                                                className="absolute right-5 top-5 text-white/4"
                                            />

                                            {/* Accent Glow */}
                                            <div className="absolute inset-0 bg-linear-to-br from-pink-500/5 via-transparent to-violet-500/5" />
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                {/* Quote Text */}
                                                <p
                                                    className="font-serif text-3xl md:text-4xl leading-[1.25] text-white"
                                                >
                                                    “{item.quote}”
                                                </p>
